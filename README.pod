...
L<Paul Vixie|"http://www.isc.org"> implemented the first reputational
data method of spam filtering.  This has become one of the most 
effective and dominantly used methods of spam protection.

With the release of BIND 9.8 a new reputational mechanism is available,
this time for use in DNS resolvers.  An organisation is able to 
receive a reputational data feed describing internet domains that
have a reputation of attacking clients that visit the domain.
...
and have left the provision of the data itself to professional 
organisations that specialise in this information.  Additionally, the
response that shall be given to a local client attempting to 
...
The second is to always respond with a CNAME which will redirect the
...
have been averted by visiting the site.
...
integrating data gathered by both the DNS resolver and visits to the
Walled Garden to allow distinguishment between human behaviour (more
precisely web browser behaviour) and malware behaviour.

=head2 Purpose

If a recursive DNS resolver is using RPZ and records in its logs 
...
data is available to be later analysed.  The tells us which systems
using the resolver are attempting to visit domains with a poor 
repuation.
...
behaviour of visiting a dangerous web site (DNS record is made and
visit to the Walled Garden is made) and the likely behavious of
locally installed malware (DNS record made, but no record at the 
Walled Garden).
...
Additionally, the project wishes to allow for entirely open access
to all data so that additional tools can be constructed to utilise
the available data according to the wishes of the organisation.
...
implementing RPZs, a Walled Garden web-site, an RDBMS and a site
which allows the organisation to view the data gathered from the 
resolvers and the Walled Garden (which has been sent to the RDBMS).

See [Open Document Graphics] L<"http://github.com/rpzla/doc/Pictorial Overview.odg">
...
* L<Taking Back the DNS | "http://www.isc.org/files/TakingBackTheDNSrpz2.pdf" >
